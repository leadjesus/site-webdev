...
Next, add [routerDirectives][] to the `@Component` annotation, and remove `HeroesComponent`:
...
Create a configuration file to hold app routes.
Define the first route as a route to the heroes component.

<?code-excerpt "lib/src/app_routes.dart (a first route)" title?>
```
...
The `AppRoutes.all` field is a list of *route definitions*. The heroes
[RouteDefinition][] has the following named arguments:
...
- `component`: The (factory of the) component that will be activated when this
  route is navigated to (`hct.HeroesComponentNgFactory`).
...
<?code-excerpt "lib/src/app_routes.dart (hct)"?>
```
  // ignore_for_file: uri_has_not_been_generated
  import 'heroes_component.template.dart' as hct;
```
...
By naming the import (`hct`) you can use the not-yet-generated template without
...
<?code-excerpt "lib/app_component_2.dart (routes and template)" title?>
```
...
is bound to a string that tells the router where to navigate when the user
clicks the link.

Since the link is not dynamic, it is defined with a one-time binding.
Looking back at the route definitions, you can confirm that
`'heroes'` is the path of the route to the `HeroesComponent`.
...
<?code-excerpt "lib/src/app_routes.dart (dashboard route)" title?>
```
  new RouteDefinition(
    path: 'dashboard',
...
<?code-excerpt "lib/src/app_routes.dart (dct)"?>
```
...
<?code-excerpt "lib/src/app_routes.dart (redirect route)" title?>
```
...
First, import the hero detail component template:

<?code-excerpt "lib/src/app_routes.dart (excerpt)" region="hdct" title?>
```
...
<?code-excerpt "lib/src/app_routes.dart (hero-detail route)" title?>
```
  new RouteDefinition(
    path: 'detail/:id',
...
Create an `updateHero()` method to extract the `id` parameter value from the
current [RouterState][], and use the `HeroService` to fetch the hero with that `id`.

<?code-excerpt "lib/src/hero_detail_component.dart (updateHero)" title?>
```
...
Notice how you can extract the `id` from the router state `parameters` map.
The hero `id` is a number. Route parameters are always strings.
So the route parameter value is converted to a number.

To get notified when a hero detail route is natigated to, make `HeroDetailComponent`
implement the [OnActivate][] interface, and call `updateHero()` from
the [onActivate()][] [router lifecycle hook][]:

<?code-excerpt "lib/src/hero_detail_component.dart (excerpt)" region="OnActivate" title?>
```
  class HeroDetailComponent implements OnActivate {
    @override
...
In `updateHero()`, you used the `getHero()` method, which `HeroService` doesn't
...
This time, you're binding to a path with a parameter.  The two segments of the
path correspond to the *name* and *:id* in the parameterized hero detail route
definition that you added earlier:

<?code-excerpt "lib/src/app_routes.dart (hero-detail route)"?>
```
  new RouteDefinition(
    path: 'detail/:id',
    component: hdct.HeroDetailComponentNgFactory,
...
[onActivate()]: /angular/guide/router/5#onactivate
